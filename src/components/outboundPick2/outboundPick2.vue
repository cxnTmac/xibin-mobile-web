  ...
  <div>
    ...
    <div v-show="pickViewVisible">
      <x-table>
        ...
        <tbody>
        <tr v-for="(item, index) in orderDetailAlloc"  @click="pickUpOrCancel(item,index)" :class="getTrClass(item)">
          <td>{{item.lineNo}}</td>
          <td>{{item.skuCode}}</td>
          <td>{{item.skuName}}</td>
...
import {XTable, Tab, TabItem, TransferDom, Popup, FormPreview, Grid, GridItem, XSwitch, Group, Cell, XInput, XButton, Alert, AlertPlugin, Divider, Panel, PopupPicker, XHeader, Search, Sticky, Loading, LoadingPlugin, Toast, ToastPlugin} from 'vux'
...
import {getOutboundDetailListPage,getOutboundAllocListPage, pick, cancelPick} from '../../api/outboundApi'
      ...
      getDetailAllocs () {
       ...
       getDetails () {
        ...
        let para = {
          ...
          conditions: JSON.stringify({orderNo: this.orderNo, multiStatus: this.detailMultiStatus})
        }
